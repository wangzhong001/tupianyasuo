...
//     */
    ...
    public static String[] image2Video(String srcDir, String format,
                                       String targetFile) {
        //-f image2：代表使用image2格式，需要放在输入文件前面
...
//    }
    ...
    public static String[] videoSpeed2(String srcFile, String targetFile){
        String command="ffmpeg -i "+srcFile+" -an -r 84 -filter:v setpts=0.25*PTS "+targetFile;
//        String command = "ffmpeg -y -i %s -filter_complex [0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a] -map [v] -map [a] %s";
//        command = String.format(command,srcFile,targetFile);
        return transcode(srcFile,targetFile);
    }

    public static String[] transcode(String srcPath, String outPath) {
...
//        command = String.format(command,srcFile,targetFile);
     ...
     * @return 音频淡入命令行
     */
    public static String[] extractAudioFeature(String srcFile, String targetFile, int start, int duration,int startOut,int durationout){
        String command = "ffmpeg -y -i %s -filter_complex afade=t=in:ss=%d:d=%d %s afade=t=out:st=%d:d=%d %s";
        command = String.format(Locale.CHINA, command,srcFile,start,duration,targetFile);
        return command.split(" ");
    }
