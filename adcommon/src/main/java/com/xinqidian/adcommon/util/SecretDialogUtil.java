...
public class SecretDialogUtil {
    ...
    public  void showPrivacy(Context context,String appname) {
        ...
        TextView mac_tv=dialog.findViewById(R.id.mac_tv);
        tv_privacy_tips.setText("欢迎使用"+appname+"！"+appname+"非常重视您的隐私和个人信息保护。在您使用"+appname+"前请认真阅读《用户协议》及《隐私政策》,您同意并接受全部条款后方可开始使用"+appname);

        mac_tv.setText("3.收集设备MAC地址等设备信息用于帮助我们分析"+appname+"在不同终端设备上的表现，以适配更多的终端设备。我们会对收集的信息进行加密，从而提高隐私的保护。如您选择不开启此权限，您将无法使用此权限特定的功能，但不影响您使用"+appname+"提供的其他服务");
        dialog.show();
